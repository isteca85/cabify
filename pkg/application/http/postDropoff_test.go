...
func createDBDropoff() *infrastructure.MockDB {
...
func TestServer_PostDropOff(t *testing.T) {
	t.Run("200 response post dropoff", func(t *testing.T) {

		serv := &application.Server{}
		serv.InitServer()
		serv.DataBase = createDBDropoff()

		data := url.Values{}
		data.Set("ID", "1")
		encodedData := data.Encode()
		req, _ := http.NewRequest(http.MethodPost, "/dropoff", strings.NewReader(encodedData))
	...
	t.Run("202 response post dropoff", func(t *testing.T) {
	...
	t.Run("404 response post dropoff", func(t *testing.T) {
	...
	t.Run("400 response post dropoff", func(t *testing.T) {
